...
def watchAdd(args):
   ...
   log.printInfo("Added " + Common.ItemToText(item) + " to the watch list.")


def watchRemove(args):
   if None == re.search("^w\d+$", args.watch_id):
      log.printError("The index_id argument [" + args.watch_id + "] is not in the expected format.\n" +
   ...
   try:
      ...
      log.printError("A number could not be extracted from the watch_id argument. Use the value on the left returned by 'watch list'")
      return

   if watchId < 0:
      log.printError("The watch_id argument is negative")
      return

   watchDict = Common.GetDictFromIndexFile(Common.C_WATCH_LIST_PATH)

   if watchId >= len(watchDict):
      log.printError("The watch_id argument [" + str(watchId) + "] is out of range [0; " + str(len(watchDict)) + ")")
      return
   ...
   log.printInfo("Removed " + Common.ItemToText(item) + " from the watch list.")
